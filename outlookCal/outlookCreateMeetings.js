...
async function createMeetingsSequentially() {
    ...
    const startISO = new Date('2024-08-06T13:40:00Z'); // example start time in ISO format
    const numberOfMeetings = 5; // number of meetings to create
    const meetingDuration = 15 * 60 * 1000; // meeting duration in milliseconds
    const attendees = [
    ...
    for (let i = 0; i < numberOfMeetings; i++) {
        const startDateTime = new Date(startISO.getTime() + i * meetingDuration).toISOString();
        const endDateTime = new Date(startISO.getTime() + (i + 1) * meetingDuration).toISOString();
